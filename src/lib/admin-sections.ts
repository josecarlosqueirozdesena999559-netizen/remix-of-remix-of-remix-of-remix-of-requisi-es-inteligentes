...
export function getAdminSections(
  ...
  return rawSections.map((value) => String(value).trim()).filter(isAdminSection);
}
...
export function getAdminSectionFromPath(pathname: string): AdminSection | null {
  ...
  if (pathname.startsWith("/admin/requisicoes-avulsas")) return "requisicoes_avulsas";
  if (pathname.startsWith("/admin/assinaturas-avulsas-assinadas")) return "requisicoes_avulsas";
