...
import { disableUserAppMode, isUserAppModeEnabled } from "@/lib/user-app-mode";
import {
...
function AdminLayout() {
  ...
  const userAppMode = isUserAppModeEnabled();
  ...
  const isAdmin = !userAppMode && profile?.is_admin !== false;
            ...
            <>
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  ...
                  {hasAdminSectionAccess(profile, "requisicoes_avulsas") ? (
                    <Link
                      to="/admin/requisicoes-avulsas"
                      className={navItemClass(isActivePath("/admin/requisicoes-avulsas"))}
                    >
                      <FileSignature
                        className={navIconClass(isActivePath("/admin/requisicoes-avulsas"))}
                      />
                      <span className="truncate">Requisições Avulsas</span>
                    </Link>
                  ) : null}
                  {hasAdminSectionAccess(profile, "requisicoes_avulsas") ? (
                    <Link
                      to="/admin/assinaturas-avulsas-assinadas"
                      className={navItemClass(isActivePath("/admin/assinaturas-avulsas-assinadas"))}
                    >
                      <FileCheck2
                        className={navIconClass(
                          isActivePath("/admin/assinaturas-avulsas-assinadas"),
                        )}
                      />
                      <span className="truncate">Controle de Assinaturas Avulsas</span>
                    </Link>
                  ) : null}
