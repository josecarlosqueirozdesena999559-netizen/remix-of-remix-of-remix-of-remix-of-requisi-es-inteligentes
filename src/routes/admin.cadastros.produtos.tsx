...
import { ImageIcon, Loader2, Save, SlidersHorizontal } from "lucide-react";
...
function renderProductImage(item: Item) {
  if (!item.imagem_url) {
    return <span className="text-muted-foreground">-</span>;
  }

  return (
    <a href={item.imagem_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-primary hover:underline">
...
function ProdutosPage() {
  ...
  const { data, loading, error } = useSupabaseList<Item>(
    "itens",
    "id,nome,unidade,categoria,subcategoria,imagem_url,programa_produtos(programas(id,nome))",
  ...
  const columns: Column<Item>[] = [
    { key: "imagem_url", label: "Imagem", render: renderProductImage },
    { key: "nome", label: "Nome" },
    {
      key: "categoria",
